...
describe('generateApiRegistry', () => {
    ...
    const registryFilePath = path_1.default.join(apiRootDir, 'registry.ts');
    beforeEach(() => {
        jest.clearAllMocks();
    });
    it('should log a warning and not write a file if no .ts files are found', async () => {
        file_system_1.walkDirectory.mockImplementation((_dir, cb) => { });
        // TODO: This is erroring in the IDE even though the function call is correct based on the signature
        // @ts-ignore
        const loggerSpy = jest.spyOn(logger_1.Logger, 'warn').mockImplementation(() => { });
        await (0, src_1.generateApiRegistry)(apiRootDir);
        expect(loggerSpy).toHaveBeenCalledWith('generateApiRegistry', 'No API files found. Registry will not be generated.');
    });
    it('should write registry file when API files are found', async () => {
        ...
        // TODO: This is erroring in the IDE even though the function call is correct based on the signature
        // @ts-ignore
        const writeFileSpy = jest.spyOn(fs_1.default, 'writeFileSync').mockImplementation(() => { });
        await (0, src_1.generateApiRegistry)(apiRootDir);
        expect(writeFileSpy).toHaveBeenCalledWith(registryFilePath, expect.stringContaining('export const apiRegistry ='), 'utf8');
...
describe('getApiFunction', () => {
    ...
    it('should return the correct function from the registry', () => {
        const fn = (0, src_1.getApiFunction)(apiRegistry, 'alpha', 'GET_User');
        expect(fn).toBe(mockApiFn);
    });
    it('should throw if service does not exist', () => {
        expect(() => (0, src_1.getApiFunction)(apiRegistry, 'unknown', 'GET_User')).toThrowError('Function "GET_User" not found in service "unknown".');
    });
    it('should throw if function does not exist in valid service', () => {
        expect(() => (0, src_1.getApiFunction)(apiRegistry, 'alpha', 'MissingFn')).toThrowError('Function "MissingFn" not found in service "alpha".');
    });
});
